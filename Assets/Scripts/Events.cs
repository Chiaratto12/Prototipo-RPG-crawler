...
public class Events : MonoBehaviour {
    ...
    public void EventsReset() {
        ...
        if (isNerfed == 0 || isBuffed > 0) {game.player.atk = game.player.classAtk + game.weaponDamage; eventsIcon.GetChild(1).gameObject.SetActive(false); isNerfed = -1;}

        isConfused --;
        if(isConfused <= 0 || clairvoyance > 0) {isConfused = 0; eventsIcon.GetChild(3).gameObject.SetActive(false);}

        clairvoyance --;
        if(clairvoyance <= 0) {clairvoyance = 0; eventsIcon.GetChild(2).gameObject.SetActive(false);}
    }
